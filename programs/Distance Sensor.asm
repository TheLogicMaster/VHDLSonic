...
; Configure T0 in repeat mode to count in ms - sets sample taking at 10Hz
    ...
    str r0, [{timer_prescale_0}]    ; write prescaler value for T0
    ldr r0, 100                     ; compare value (100ms)
    str r0, [{timer_repeat_0}]      ; write repeat value

; Configure T1 in one-shot mode for trigger
    ldr r0, 50                      ; prescaler
    str r0, [{timer_prescale_1}]    ; write prescale value for T1
    ldr r0, 10                      ; trigger duration
    str r0, [{timer_compare_1}]     ; write one-shot duration

; Configure GPIO - P0 used for output; P1 for input set by default
    ldr r0, 1
    str r0, [{gpio_modes_0}]        ; set P0 to output (1)

loop:
    ldr r0, 1
    str r0, [{timer_enable_0}]      ; enable T0
    str r0, [{gpio_0}]              ; set P0 high
    str r0, [{timer_enable_1}]      ; enable T1
    ldr r3, 500
...
sampling:
    ldr r0, [{timer_count_0}]       ; check if T0 has rolled over
    bne sampling
    bra loop
    data
    align 4
